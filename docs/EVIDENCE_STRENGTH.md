...
Unknown citation IDs and uncited findings fail validation. Citation validation is an allowlist check, not semantic entailment. The current pipeline parses responses, renders exact facts deterministically and constrains generated context to evidence vocabulary. These conservative checks are implemented, but are not a general natural-language entailment guarantee. Recognized requests for operational procedures/limits fail closed because no approved asset-specific authorization corpus exists. See the remediation ledger for tested boundaries.
